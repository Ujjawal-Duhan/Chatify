...
const Dashboard = () => {
	...
	return (
		<div className='w-screen flex'>
			<div className='w-[25%] h-screen overflow-auto  bg-cyan-50'>
				<div className='flex items-center my-8 mx-14'>
					<div><img src={Avatar} width={75} height={75} className='border border-blue-400 p-[2px] rounded-full' /></div>
					<div className='ml-8'>
									...
									return (
										<div className='flex items-center py-8 border-b border-b-gray-300'>
											<div className='cursor-pointer flex items-center' onClick={() => fetchMessages(conversationId, user)}>
												<div><img src={Avatar} className="w-[40px] h-[40px] rounded-full p-[2px] border border-blue-400" /></div>
												<div className='ml-6'>
			...
			<div className='w-[50%] h-screen bg-white flex flex-col items-center'>
				{
					...
					<div className='w-[75%] bg-gray-100 h-[65px] my-14 rounded-full flex items-center px-14 py-2'>
				...
				<div className='h-[400px] w-full overflow-auto shadow-sm'>
					<div className='p-14'>
						{
							messages?.messages?.length > 0 ?
								messages.messages.map(({ message, user: { id } = {} }) => {
									return (
										<>
										<div className={`max-w-[40%] rounded-b-xl p-4 mb-6 ${id === user?.id ? ' text-white rounded-tl-xl ml-auto bg-neutral-700' : 'rounded-tr-xl bg-gray-300'} `}>{message}</div>
				...
				{
					messages?.receiver?.fullName &&
					<div className='p-14 w-full h-[15px] flex items-center'>
			...
			<div className='w-[25%] h-screen bg-orange-50 px-8 py-16 overflow-scroll'>
								...
								return (
									<div className='flex items-center py-8 border-b border-b-gray-300'>
										<div className='cursor-pointer flex items-center' onClick={() => fetchMessages('new', user)}>
											<div><img src={Avatar} className="w-[60px] h-[60px] rounded-full p-[2px] border border-blue-400" /></div>
											<div className='ml-6'>
												<h3 className='text-lg font-semibold'>{user?.fullName}</h3>
												<p className='text-sm font-light text-gray-600'>{user?.email}</p>
											</div>
